...
class ProphecyUtility {
  ...
  Map<ProphecyType, ProphecyEntity> calculate({@required int dt, bool isDebug = false, @required UserEntity user}) {
    final limitedResult = limitProphecies(
      //
  ...
  double compatibility({
    ...
    result += calcUser[ProphecyType.ROOT].value * 0.1 + calcSubj[ProphecyType.ROOT].value * 0.1;

    /// overall happiness (sacral chakra)
    result += calcUser[ProphecyType.SACRAL].value * 0.1 + calcSubj[ProphecyType.SACRAL].value * 0.1;

    /// how positive are they today (heart chakra)
    result += calcUser[ProphecyType.HEART].value * 0.3 + calcSubj[ProphecyType.HEART].value * 0.3;

    /// it creates conflicts (throat chakra)
    result -= calcUser[ProphecyType.THROAT].value * 0.11 + calcSubj[ProphecyType.THROAT].value * 0.11;
    ...
    if (user.model.sex == 1) {
      ...
      if (subject.model.sex != 1) {
        ...
        result += calcUser[ProphecyType.SACRAL].value * 0.11 + calcSubj[ProphecyType.SACRAL].value * 0.11;
      } else {
        /// with other males
        result -= calcUser[ProphecyType.SOLAR].value * 0.14;
      }
