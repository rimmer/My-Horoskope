...
class PropheciesBloc extends Bloc<ProphecyEvent, ProphecyState> {
  ...
  PropheciesBloc({@required repository}): assert(repository != null),
  ...
  Stream<ProphecyState> mapEventToState(
    ...
    switch (event.runtimeType) {
      case LoadProphecies: _mapLoadPropheciesToState(); break;
      case PropheciesLoaded: yield* _mapPropheciesLoadedToState(event);
    }
  }

  void _mapLoadPropheciesToState() {
    _prophecySubscription?.cancel();
    _prophecySubscription = _repository.prophecies().listen(
            (prophecies) => add(PropheciesLoaded(prophecies))
    );
  }

  Stream<ProphecyState> _mapPropheciesLoadedToState(PropheciesLoaded event) async* {
